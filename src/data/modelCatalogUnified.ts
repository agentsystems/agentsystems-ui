  ...
  aws_credentials: {
    ...
    fields: [
      ...
      },
      {
        ...
        required: true,
        helpText:
          "Environment variable containing your AWS region (e.g., us-east-1, us-west-2)",
      },
      {
        name: "regionPrefix",
        label: "Region Prefix",
        ...
        required: false,
        helpText:
          'Region prefix for model IDs (e.g., "us", "eu") - leave blank if not needed',
      },
    ],
  ...
  ollama_auth: {
    ...
    fields: [
      ...
      },
      {
        ...
        required: false,
        helpText:
          "Only needed for authenticated remote Ollama deployments behind reverse proxies",
      },
    ],
    helpText:
      "Configure connection to Ollama server with optional authentication",
  },
...
export const MODELS: Record<string, ModelDefinition> = {
  ...
  "claude-opus-4-1": {
    ...
    category: "multimodal",
    description:
      "Most advanced Claude model with enhanced reasoning capabilities",
    hostingProviders: [
      {
        id: "anthropic",
  ...
  "claude-sonnet-4-5": {
    ...
    category: "text",
    description:
      "Most advanced Claude Sonnet with enhanced capabilities for complex agents and coding",
    hostingProviders: [
      {
        id: "anthropic",
        displayName: "Anthropic",
    ...
    hostingProviders: [
      {
        id: "ollama",
        displayName: "Ollama",
        hostingProviderModelId: "gpt-oss:20b",
        authMethod: "ollama_auth",
      },
    ],
  },
...
export function getModelsGroupedByVendor(): Record<
  ModelVendor,
  ModelDefinition[]
> {
  ...
  Object.keys(grouped).forEach((vendor) => {
    grouped[vendor as ModelVendor]!.sort((a, b) =>
...
export function getHostingProvidersForModel(
  modelId: string
...
export function getHostingProviderModelId(
  modelId: string,
  ...
  const provider = model.hostingProviders.find(
    (p) => p.id === hostingProviderId
  );
  return provider?.hostingProviderModelId;
}

export function getHostingProvider(
  hostingProviderId: HostingProviderId
): HostingProviderSupport | undefined {
